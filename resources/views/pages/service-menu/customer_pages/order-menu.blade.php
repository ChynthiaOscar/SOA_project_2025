...
<div class="min-h-screen bg-[#131313] pt-16 md:pl-[25%] xl:pl-[15%] relative overflow-x-hidden">
    ...
    <aside class="hidden md:flex fixed top-0 left-0 h-screen w-[25%] xl:w-[15%] bg-[#E2BB4D] items-center justify-end py-10 z-20" style="clip-path: ellipse(92% 70% at 0% 50%); overflow: visible;">
        <aside class="fixed top-0 left-0 h-screen w-[25%] xl:w-[15%] bg-[#65090D] text-[#E2BB4D] flex items-center justify-end pr-6 py-10" style="clip-path: ellipse(90% 70% at 0% 50%); overflow: visible;">
            <div class="relative w-[300px] h-[300px] mx-auto mt-10">
                <ul id="circle" class="absolute inset-0 flex items-center justify-center">
                    @foreach (['Appetizer', 'Soup', 'Dimsum', 'Main Course', 'Dessert', 'Beverages', 'Wine'] as $item)
                        <li onclick="rotateTo(this)" class="menu-item absolute font-serif hover:text-[#E09121] cursor-pointer text-right" style="text-shadow: 1px 1px 5px #000;">{{ $item }}</li>
                    @endforeach
                </ul>
    ...
    <div class="md:hidden bg-[#65090D] text-[#E2BB4D] overflow-x-auto whitespace-nowrap flex gap-6 px-4 py-3">
        @foreach (['Appetizer', 'Soup', 'Dimsum', 'Main Course', 'Dessert', 'Beverages', 'Wine'] as $item)
            <span class="font-serif hover:text-[#E09121] cursor-pointer text-sm whitespace-nowrap">{{ $item }}</span>
        @endforeach
    </div>

    <!-- Search bar -->
   <div class="w-full pt-6 sm:pt-10 md:pt-6 px-4">
    <div class="border border-[#E2BB4D] px-3 sm:px-4 py-2 flex items-center gap-2 sm:gap-3 w-full max-w-xs sm:max-w-md md:max-w-xl mx-auto bg-[#131313]">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#E2BB4D" class="w-4 h-4 sm:w-5 sm:h-5">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20 md:mt-24">
    ...
    <div class="fixed bottom-10 right-10 z-30">
        <a href="#" class="bg-[#65090D] px-3 py-3 rounded-full shadow-lg hover:bg-[#7F160C] transition duration-300 border-2 border-[#E2BB4D] flex justify-center items-center">
            <svg xmlns="http://www.w3.org/2000/svg" height="36px" viewBox="0 -960 960 960" width="40px" fill="#E2BB4D">
                <path d="M80-200v-66.67h800V-200H80Zm33.33-120v-24.67q0-146 85.17-242T413.33-710v-23.33q0-28.34 19.17-47.5Q451.67-800 480-800t47.5 19.17q19.17 19.16 19.17 47.5V-710q130 27.33 215 123.33t85 242V-320H113.33Z"/>
            </svg>    
        </a>
    </div>
</div>
