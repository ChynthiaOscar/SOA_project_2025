...
@section('content')
    <div class="min-h-screen bg-[#131313] pt-16 md:pl-[25%] xl:pl-[15%] relative overflow-x-hidden">
                    ...
                    <ul id="circle" class="absolute inset-0 flex items-center justify-center">
                        @foreach ($categories as $category)
                            <li onclick="rotateTo(this)"
                                ...
                                data-category-id="{{ $category['id'] }}" style="text-shadow: 1px 1px 5px #000;">
                                {{ $category['name'] }}</li>
                        @endforeach
                    </ul>
                    <div
        ...
        <div class="md:hidden bg-[#65090D] text-[#E2BB4D] overflow-x-auto whitespace-nowrap flex gap-6 px-4 py-3">
            @foreach ($categories as $category)
                <span class="font-serif hover:text-[#E09121] cursor-pointer text-sm whitespace-nowrap"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20 md:mt-24">
            <div id="menu-grid"
                class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-6 gap-y-16 justify-items-center mb-8">
    ...
    <script>
        ...
        function renderMenus(activeCategoryId) {
            const wrappers = document.querySelectorAll('.menu-card-wrapper');
            let found = false;
            wrappers.forEach(wrapper => {
                if (wrapper.dataset.categoryId == activeCategoryId) {
            ...
            if (!found) {
                document.getElementById('menu-grid').innerHTML =
                    '<p class="text-[#E2BB4D] text-center col-span-3">No menu available for this category.</p>';
            }
        }
        ...
        const defaultActiveCategoryId = items[3]?.dataset.categoryId;
        if (defaultActiveCategoryId) {
            renderMenus(defaultActiveCategoryId);
        }
