    ...
    <div class="bg-[#f7ebc6] border-4 border-[#8b0000] rounded-lg shadow-xl p-8 w-full max-w-5xl">
        ...
        <form id="reservationForm" method="POST" action="{{ route('event-reservations.store') }}">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date" class="w-full border-2 border-[#8b0000] rounded px-3 py-2" required>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-6">
                ...
                <input type="number" name="total_price" class="w-full border-2 border-[#8b0000] rounded px-3 py-2" min="0" step="0.01" placeholder="Enter total price">
            </div>
            ...
            @if(!empty($categories))
                @foreach($categories as $category)
                    @if(isset($category->name) && isset($category->event_menus) && is_array($category->event_menus))
                        <div class="mb-10">
                            <h2 class="text-xl font-extrabold text-[#8b0000] mb-4 uppercase border-b-2 border-[#8b0000]">{{ $category->name }}</h2>
                            <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
                                @foreach($category->event_menus as $menu)
                                    <label class="relative group rounded-lg shadow-lg overflow-hidden border-4 border-yellow-700 bg-[#8b0000] text-white hover:scale-105 transition-all cursor-pointer">
                                        ...
                                        <div class="p-3 text-center">
                                            ...
                                            <div class="mt-1 font-semibold text-yellow-300">Rp {{ number_format($menu->price, 0, ',', '.') }}</div>
                                        </div>
                                    </label>
                                @endforeach
                            </div>
