...
<div class="min-h-screen bg-yellow-50 flex items-center justify-center py-10">
    <div class="bg-slate-200 rounded-lg shadow p-8 w-full max-w-md flex flex-col gap-2">
        ...
        <div>
            ...
            <input type="text" id="name" class="w-full border border-black rounded px-3 py-2" value="{{ $event_menu->name }}">
        </div>

        <div>
            <label class="block font-semibold mb-1">Price</label>
            <input type="number" id="price" class="w-full border border-black rounded px-3 py-2" value="{{ $event_menu->price }}">
        </div>
        ...
        <div>
            ...
            <select id="dish_category_id" class="w-full border border-black rounded px-3 py-2" required>
                @foreach($categories as $cat)
                    <option value="{{ $cat->id }}" {{ $event_menu->dish_category_id == $cat->id ? 'selected' : '' }}>
                        {{ $cat->name }}
                    </option>
                @endforeach
            </select>
