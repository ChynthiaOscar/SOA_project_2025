@extends('layouts.app')
@section('title', 'Create Event Package')
@section('content')
    <div class="min-h-screen bg-yellow-50 flex items-center justify-center py-10">
        <div class="bg-slate-200 rounded-lg shadow p-8 w-full max-w-md flex flex-col gap-2">
            <h1 class="text-xl font-bold text-yellow-900 mb-6">Create Event Package</h1>
            <div>
            ...
            <div>
                ...
                <textarea id="description" class="w-full border border-black rounded px-3 py-2" rows="3" required></textarea>
            </div>
            <div>
                <label class="block font-semibold mb-1">Pax</label>
                <input type="number" id="pax" class="w-full border border-black rounded px-3 py-2" required>
            </div>
            <div>
            ...
            <div>
                ...
                <select id="event_space_id" class="w-full border border-black rounded px-3 py-2" required>
                    ...
                    @foreach($eventSpaces as $eventSpace)
                        <option value="{{ $eventSpace->id }}">{{ $eventSpace->id }} - {{ $eventSpace->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex justify-end gap-2 mt-4">
                <a href="{{ url('event-packages') }}"
    ...
    <script>
        document.getElementById("submit").addEventListener("click", () => {
            Swal.fire({
                icon: "warning",
                title: "Warning",
                text: "Are you sure to create this Event Package data?",
                showDenyButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`{{ route('event-packages.store') }}`, {
                        method: 'POST',
                        headers: {
