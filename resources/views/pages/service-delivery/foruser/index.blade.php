...
<body class="bg-darkbg text-cream min-h-screen">
     <div class="container mx-auto px-4 py-10 max-w-3xl">
        ...
        @else
                    ...
                    @if(count($orderDetails) > 0)
                        @foreach($orderDetails as $detail)
                            <div class="flex justify-between mb-2">
                                <span>{{ $detail['menu_name'] ?? 'Menu Item' }} x{{ $detail['order_detail_quantity'] }}</span>
                                <span>Rp {{ number_format($detail['menu_price'] * $detail['order_detail_quantity']) }}</span>
                            </div>
                        ...
                        <div class="flex justify-between font-bold text-gold-light">
                            ...
                            <span>Rp {{ number_format(array_sum(array_map(fn($detail) => $detail['menu_price'] * $detail['order_detail_quantity'], $orderDetails))) }}</span>
                        </div>
                    @else
            ...
            <!-- Delivery Form -->
            <form id="deliveryForm" class="mb-8 bg-black/30 p-6 rounded-lg border border-tan/20">
            ...
            <div id="success_message" class="hidden text-center py-10">
                ...
                <a href="/" class="bg-maroon hover:bg-maroon-dark text-cream font-bold py-2 px-6 rounded transition duration-300">
                    Return Home
                </a>
            </div>
        @endif
...
<script>
    ...
    if (deliveryForm) {
        deliveryForm.addEventListener('submit', function(e) {
            ...
            if (!formData.destination || !formData.lat || !formData.lon || !formData.distance || !formData.price) {
    ...
    function renderSearchResults(results) {
        ...
        results.forEach(place => {
            ...
            <div class="p-3 hover:bg-black/30 cursor-pointer border-b border-tan/20" 
                ...
                <div class="font-bold">${place.display_name}</div>
                <div class="text-xs text-tan">${place.type}: ${place.lat}, ${place.lon}</div>
            </div>`;
        });
    ...
    function getDistance(lat, lon) {
        ...
        .then(result => {
            ...
            // Display distance and price info
            distanceValue.textContent = result.distance + ' km';
            priceValue.textContent = 'Rp ' + numberWithCommas(result.price);
            
            // Set hidden inputs for form submission
            distanceInput.value = result.distance;
            priceInput.value = result.price;
            
            // Show distance info section
        ...
        .catch(error => {
            ...
            showNotification('Failed to calculate distance', 'error');
        });
    }
